...
// Export this function so server.js can register the broadcast function
  ...
  (async function loop() {
    while (alive) {
      try {
        ...
        if (fire) {
          log.warn({ cam: cam.camera, userId: cam.userId }, "FIRE DETECTED");
        } else {
          log.info({ cam: cam.camera, fire }, "No fire detected");
        }
        
        // Broadcast fire status via WebSocket if function is registered
        if (broadcastFunction) {
          broadcastFunction(cam.userId, cam.id, cam.camera, fire);
        }
